...
app.listen(PORT, '0.0.0.0', () => {
  ...
  migrate().then(async () => {
    ...
    // Merge roster file (idempotent) on every boot unless SKIP_ROSTER_FILE=1
    try {
      if (process.env.SKIP_ROSTER_FILE === '1') {
        console.log('[SEED] SKIP_ROSTER_FILE=1, se omite merge de roster')
      } else {
        const fs = await import('node:fs/promises')
        const rosterPath = path.join(__dirname, '..', 'seeds', 'engineers.import.json')
        let raw: any = null
        try { raw = JSON.parse(await fs.readFile(rosterPath, 'utf8')) } catch (e) {
          console.warn('[SEED] No se pudo leer roster', (e as any)?.message)
        }
        if (Array.isArray(raw) && raw.length) {
          const total = raw.length
            ...
            try {
              ...
              console.log(`[SEED] Merge roster completado (${upserts}/${total})`)
            } catch (e) {
              if (isPg) { try { await pool.query('ROLLBACK') } catch {} }
              console.error('[SEED] Error merge roster', (e as any)?.message)
            }
        } else {
          console.warn('[SEED] Roster vacío o inválido (no merge)')
        }
      }
    } catch (e) {
      console.warn('[SEED] Falló merge de roster', (e as any)?.message)
    }
